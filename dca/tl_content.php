...
class tl_content_modal extends Backend
{
    ...
    public function checkPermission()
    {
        ...
        switch (Input::get('act'))
        {
            ...
            case 'copyAll':
                ...
                $objCes = $this->Database->prepare("SELECT id FROM tl_content WHERE ptable='tl_modal' AND pid=?")->execute(CURRENT_ID);
    ...
    protected function checkAccessToElement($id, $root, $blnIsPid = false)
    {
        ...
        {
            $objArchive =
                $this->Database->prepare("SELECT a.id, m.id AS mid FROM tl_modal m, tl_modal_archive a WHERE m.id=? AND m.pid=a.id")->limit(1)->execute($id);
        }
        else
        {
            $objArchive = $this->Database->prepare(
                "SELECT a.id, m.id AS mid FROM tl_content c, tl_modal m, tl_modal_archive a WHERE c.id=? AND c.pid=m.id AND m.pid=a.id"
            )->limit(1)->execute($id);
        }
    ...
    public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return '<a href="' . $this->addToUrl($href) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml(
            $icon,
            $label,
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        $objVersions->create();
        $this->log(
            'A new version of record "tl_content.id=' . $intId . '" has been created' . $this->getParentEntries('tl_content', $intId),
            __METHOD__,
            TL_GENERAL
        );
    }
